<%@ Page Title="" Language="C#" MasterPageFile="~/MainMaster.master" AutoEventWireup="true" CodeBehind="CostReport_ReceivablesPaymentManage.aspx.cs" Inherits="ERP.UI.Web.CostReport.CostReport_ReceivablesPaymentManage" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="CPHStage" runat="server">
    ...
    <table style="width: 100%;">
        ...
        <tr>
            <td colspan="8" style="text-align: center;">
                ...
                <asp:Button ID="btn_PayAndReceive" runat="server" Text="批量收/付款" OnClick="btn_PayAndReceive_Click" />
    ...
    <rad:RadGrid ID="RG_Report" runat="server" ShowFooter="true" OnNeedDataSource="RG_Report_NeedDataSource" OnItemDataBound="RG_Report_ItemDataBound">
        <MasterTableView DataKeyNames="PayBankAccountId,AssumeFilialeId">
            <CommandItemTemplate>
            </CommandItemTemplate>
            ...
            <Columns>
                <rad:GridTemplateColumn HeaderText="&lt;input type=checkbox value=-1 onclick=checkAll()&gt;全选">
                    <ItemTemplate>
                        <input title="请选择" type="checkbox" name="ckId" value='<%# Eval("ReportId")+"&"+Eval("State")+"&"+Eval("ReportNo")%>' <%# (string.IsNullOrEmpty(Eval("DepositNo").ToString())?((Eval("PayBankAccountId").Equals(Guid.Empty)||Eval("AssumeFilialeId").Equals(Guid.Empty))||(!Eval("State").Equals((int)CostReportState.AlreadyAuditing)&&!Eval("State").Equals((int)CostReportState.WaitVerify))):(!(Eval("State").Equals((int)CostReportState.WaitVerify)&&Hid_Flag.Value.Equals("1"))))?"disabled":""%> /><span <%# (string.IsNullOrEmpty(Eval("DepositNo").ToString())?((Eval("PayBankAccountId").Equals(Guid.Empty)||Eval("AssumeFilialeId").Equals(Guid.Empty))||(!Eval("State").Equals((int)CostReportState.AlreadyAuditing)&&!Eval("State").Equals((int)CostReportState.WaitVerify))):(!(Eval("State").Equals((int)CostReportState.WaitVerify)&&Hid_Flag.Value.Equals("1"))))?"style='color:red;'":""%>>选择</span>
                    </ItemTemplate>
                ...
                <rad:GridTemplateColumn HeaderText="操作">
                    <ItemTemplate>
                        <%#(Eval("State").Equals((int)CostReportState.AlreadyAuditing)||Eval("State").Equals((int)CostReportState.WaitVerify))?"<input type=\"button\" value=\""+(Eval("State").Equals((int)CostReportState.AlreadyAuditing)?"付款":"收款")+"\" onclick=\"ExecuteCost('"+Eval("ReportId")+"','"+Eval("DepositNo")+"')\" />":"<input type=\"button\" value=\"查看\" onclick=\"Look('"+Eval("ReportId")+"')\" />"%>
                    </ItemTemplate>
                    <HeaderStyle HorizontalAlign="Center" />
                    <ItemStyle Width="40px" HorizontalAlign="Center" />
                </rad:GridTemplateColumn>
    ...
    <rad:RadWindowManager runat="server" OnAjaxRequest="RAMPositionPower_AjaxRequest">
        <Windows>
            <rad:RadWindow ID="raw" runat="server" Title="申报收/付款" Width="1000px" Height="650px" />
    ...
    <div id="divContent" style="display: none; position: absolute; _position: absolute; z-index: 100; border: solid 1px #718CA1; background-color: #F1F1F1; width: 400px; font-size: 13px;">
        <table style="width: 100%;" cellpadding="0" cellspacing="0">
            <tr style="height: 27px; background-image:url(../App_Themes/title.png)">
                <td style="cursor: move; text-align: center; color: white; font-weight: bolder;" valign="middle">批量收/付款</td>
    ...
    <script type="text/javascript">
        ...
        //付款
        function ExecuteCost(reportId, depositNo) {
            if (depositNo.length > 0) {
                window.radopen("../CostReport/CostReport_ReceivablesPaymentFeeIncomeDepositRecovery.aspx?ReportId=" + reportId, "raw");
            } else {
                window.radopen("../CostReport/CostReport_ReceivablesPayment.aspx?ReportId=" + reportId, "raw");
            }
        }
        ...
        //是否允许押金回收生成的费用收入执行批量操作(1:允许)
        function SetFlag() {
            $("input[id$='Hid_Flag']").val("1");
        }
    </script>
</asp:Content>
