<%@ Page Title="" Language="C#" MasterPageFile="~/MainMaster.master" AutoEventWireup="true" CodeBehind="CostReport_AuditManage.aspx.cs" Inherits="ERP.UI.Web.CostReport.CostReport_AuditManage" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="CPHStage" runat="server">
    ...
    <table style="width: 100%;">
        ...
        <tr>
            ...
            <td>
                <asp:TextBox ID="txt_ReportCostStart" runat="server" Width="70px"></asp:TextBox>
                至
                <asp:TextBox ID="txt_ReportCostEnd" runat="server" Width="70px"></asp:TextBox>
            </td>
            <td style="text-align: right;">紧急程度：
            </td>
            <td>
                <asp:RadioButtonList ID="rbl_UrgentOrDefer" runat="server" RepeatDirection="Horizontal" Style="float: left;">
                    <asp:ListItem Text="普通" Value="0"></asp:ListItem>
                    <asp:ListItem Text="加急" Value="1" Selected="True"></asp:ListItem>
                </asp:RadioButtonList>
            </td>
        ...
        <tr>
            <td colspan="8" style="text-align: center;">
                ...
                <asp:Button ID="btn_Audit" runat="server" Text="批量审批" OnClick="btn_Audit_Click" />
        ...
        <MasterTableView>
            ...
            <Columns>
                <rad:GridTemplateColumn HeaderText="&lt;input type=checkbox value=-1 onclick=checkAll()&gt;全选">
                    <ItemTemplate>
                        <input title="请选择" type="checkbox" name="ckId" value='<%# Eval("ReportId")+"&"+Eval("State")+"&"+Eval("ReportNo")%>' <%# Eval("State").Equals((int)CostReportState.Auditing)&&DepositRecoveryLater(Convert.ToInt32(Eval("ReportKind")),new Guid(Eval("ReportId").ToString()))?"":"disabled"%> /><span <%# Eval("State").Equals((int)CostReportState.Auditing)&&DepositRecoveryLater(Convert.ToInt32(Eval("ReportKind")),new Guid(Eval("ReportId").ToString()))?"":"style='color:red;'"%>>选择</span>
                    </ItemTemplate>
                ...
                <rad:GridTemplateColumn HeaderText="审批时间">
                    <ItemTemplate>
                ...
                <rad:GridTemplateColumn HeaderText="已打款金额" UniqueName="PayCost">
                    <ItemTemplate>
                ...
                <rad:GridTemplateColumn HeaderText="处理状态">
                    <ItemTemplate>
                        <%#EnumAttribute.GetKeyName((CostReportState)Convert.ToInt32(Eval("State")))%>
                    </ItemTemplate>
                    <HeaderStyle HorizontalAlign="Center" />
                    <ItemStyle Width="60px" HorizontalAlign="Center" />
                </rad:GridTemplateColumn>
                ...
                <rad:GridTemplateColumn HeaderText="操作">
                    <ItemTemplate>
                        <%#(Eval("State").Equals((int)CostReportState.Auditing)&&(ReportList.Any()&&ReportList.Any(p => p.ReportId.Equals(Eval("ReportId")))))?"<input type=\"button\" value=\"审批\" onclick=\"Auditing('"+Eval("ReportId")+"','"+Eval("ReportKind")+"')\" />":"<input type=\"button\" value=\"查看\" onclick=\"Look('"+Eval("ReportId")+"')\" />"%>
                    </ItemTemplate>
                    <HeaderStyle HorizontalAlign="Center" />
                    <ItemStyle Width="40px" HorizontalAlign="Center" />
                </rad:GridTemplateColumn>
    ...
    <rad:RadWindowManager runat="server" OnAjaxRequest="RAMPositionPower_AjaxRequest">
        <Windows>
            <rad:RadWindow ID="raw" runat="server" Title="费用审批" Width="1000px" Height="650px" />
    ...
    <div id="divContent" style="display: none; position: absolute; _position: absolute; z-index: 100; border: solid 1px #718CA1; background-color: #F1F1F1; width: 400px; font-size: 13px;">
        <table style="width: 100%;" cellpadding="0" cellspacing="0">
            <tr style="height: 27px; background-image:url(../App_Themes/title.png)">
                <td style="cursor: move; text-align: center; color: white; font-weight: bolder;" valign="middle">批量审批</td>
            ...
            <tr style="height: 35px;">
                <td colspan="2" style="text-align: center; padding-bottom: 5px;">
                    <asp:Button ID="btn_Pass" runat="server" Text="通&nbsp;&nbsp;过" OnClick="btn_Pass_Click" />
                </td>
            </tr>
        </table>
    </div>
    <script type="text/javascript">
        ...
        //审核
        function Auditing(reportId, reportKind) {
            window.radopen("../CostReport/CostReport_Audit.aspx?ReportId=" + reportId + "&ReportKind=" + reportKind, "raw");
        }
