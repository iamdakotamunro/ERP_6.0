...
{
    public partial class CostReport_Paying : WindowsPage
    {
        ...
        protected void rcb_PayBankAccount_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            ddl_InvoiceTitle.SelectedValue = string.Empty;
            if (!string.IsNullOrEmpty(selectedValue) && new Guid(selectedValue) != Guid.Empty)
            {
                ...
                #region “票据类型”是发票时，“结算账号”必须是公司所属“结算账号”。根据“结算公司Id”判断所选“结算账号”是否是公司所属“结算账号”
                ...
                {
                    var list = FilialeInfoList.Select(p => p.ID);
                    if (!list.Contains(new Guid(Hid_AssumeFiliale.Value)))
                    {
                        MessageBox.Show(this, "请选择公司所属“结算账号”！");
                        rcb_PayBankAccount.SelectedValue = string.Empty;
                ...
                #endregion
                ddl_InvoiceTitle.SelectedValue = Hid_AssumeFiliale.Value.ToLower();
            }
            else
            {
                txt_AssumeFiliale.Text = string.Empty;
        ...
        protected void EditCostReportModel(CostReportInfo model, PersonnelInfo personnelInfo)
        {
            ...
            model.ReportKind = (int)CostReportKind.Paying;
            ...
            model.Memo = WebControl.RetrunUserAndTime("[【已扣款核销】：" + txt_ReportMemo.Text + ";]");
        ...
        protected string CheckData()
        {
            ...
            if (!string.IsNullOrEmpty(rblCostType) && rblCostType.Equals("2"))
            {
                ...
                {
                    errorMsg.Append("请填写“付款银行”！").Append("\\n");
                }
                else if (bankName.IndexOf("银行", StringComparison.Ordinal) > -1)
                {
                    ...
                    {
                        errorMsg.Append("请填写“付款支行”！").Append("\\n");
                    }
                }
            }
            #endregion
            ...
            #region 如果“票据类型”是“发票”，则“结算账号”必须是公司所属“结算账号”;
            if (!string.IsNullOrEmpty(rcb_PayBankAccount.SelectedValue))
            {
                if (int.Parse(rbl_InvoiceType.SelectedValue).Equals((int)CostReportInvoiceType.Invoice) || int.Parse(rbl_InvoiceType.SelectedValue).Equals((int)CostReportInvoiceType.VatInvoice))
                {
                    ...
                    if (!(bankAccountInfo != null && bankAccountInfo.IsMain))
                    {
                        errorMsg.Append("请选择公司所属“结算账号”！").Append("\\n");
                    }
                }
            }
            #endregion
        ...
        protected void InvoiceTypeChange()
        {
            if (int.Parse(rbl_InvoiceType.SelectedValue).Equals((int)CostReportInvoiceType.Invoice) || int.Parse(rbl_InvoiceType.SelectedValue).Equals((int)CostReportInvoiceType.VatInvoice))
            {
                ...
                #region “票据类型”是发票时，“结算账号”必须是公司所属“结算账号”。根据“结算公司Id”判断所选“结算账号”是否是公司所属“结算账号”
                if (!string.IsNullOrEmpty(Hid_AssumeFiliale.Value))
                {
                    if (int.Parse(rbl_InvoiceType.SelectedValue).Equals((int)CostReportInvoiceType.Invoice) || int.Parse(rbl_InvoiceType.SelectedValue).Equals((int)CostReportInvoiceType.VatInvoice))
                    {
                        var list = FilialeInfoList.Select(p => p.ID);
