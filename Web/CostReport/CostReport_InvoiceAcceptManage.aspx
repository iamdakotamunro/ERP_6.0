<%@ Page Title="" Language="C#" MasterPageFile="~/MainMaster.master" AutoEventWireup="true" CodeBehind="CostReport_InvoiceAcceptManage.aspx.cs" Inherits="ERP.UI.Web.CostReport.CostReport_InvoiceAcceptManage" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="CPHStage" runat="server">
    ...
    <table style="width: 100%;">
        ...
        <tr>
            ...
            <td style="text-align: right;">票据号码：
            </td>
            <td>
                <asp:TextBox ID="txt_InvoiceAndReceiptNo" runat="server"></asp:TextBox>
            </td>
            <td style="text-align: right;">申报类型：
            </td>
            <td>
                <asp:DropDownList ID="ddl_ReportKind" runat="server">
                </asp:DropDownList>
            </td>
        ...
        <tr>
            <td colspan="8" style="text-align: center;">
                ...
                <asp:Button ID="btn_Accept" runat="server" Text="批量受理" OnClick="btn_Accept_Click" />
        ...
        <MasterTableView>
            ...
            <Columns>
                <rad:GridTemplateColumn HeaderText="&lt;input type=checkbox value=-1 onclick=checkAll()&gt;全选">
                    <ItemTemplate>
                        <input title="请选择" type="checkbox" name="ckId" value='<%# Eval("ReportId")+"&"+Eval("State")+"&"+Eval("ReportNo")%>' <%# ((ReportList.Any()&&ReportList.Any(p => p.ReportId.Equals(Eval("ReportId"))))&&(Eval("State").Equals((int)CostReportState.NoAuditing)))?"":"disabled"%> /><span <%# ((ReportList.Any()&&ReportList.Any(p => p.ReportId.Equals(Eval("ReportId"))))||(Eval("State").Equals((int)CostReportState.NoAuditing)))?"":"style='color:red;'"%>>选择</span>
                    </ItemTemplate>
                ...
                <rad:GridTemplateColumn HeaderText="受理时间">
                    <ItemTemplate>
                        <%#Convert.ToDateTime(Eval("AcceptDate")).ToString("yyyy-MM-dd").Equals("1900-01-01")?"":Convert.ToDateTime(Eval("AcceptDate")).ToString("yyyy-MM-dd") %>
                    </ItemTemplate>
                    <HeaderStyle HorizontalAlign="Center" />
                    <ItemStyle Width="65px" HorizontalAlign="Center" />
                </rad:GridTemplateColumn>
                ...
                <rad:GridTemplateColumn HeaderText="票据号码">
                    <ItemTemplate>
                        <span style="word-break: break-all;"><%#string.IsNullOrEmpty(Eval("InvoiceNo").ToString())?Eval("ReceiptNo"):Eval("InvoiceNo")  %></span>
                    </ItemTemplate>
                    <HeaderStyle HorizontalAlign="Center" />
                ...
                <rad:GridTemplateColumn HeaderText="操作">
                    <ItemTemplate>
                        <%#((ReportList.Any()&&ReportList.Any(p => p.ReportId.Equals(Eval("ReportId"))))&&(Eval("State").Equals((int)CostReportState.NoAuditing)))?"<input type=\"button\" value=\"受理\" onclick=\"Accept('"+Eval("ReportId")+"')\" />":"<input type=\"button\" value=\"查看\" onclick=\"Look('"+Eval("ReportId")+"')\" />"%>
                    </ItemTemplate>
                    <HeaderStyle HorizontalAlign="Center" />
                    <ItemStyle Width="40px" HorizontalAlign="Center" />
                </rad:GridTemplateColumn>
    ...
    <rad:RadWindowManager runat="server" OnAjaxRequest="RAMPositionPower_AjaxRequest">
        <Windows>
            <rad:RadWindow ID="raw" runat="server" Title="票据受理" Width="1000px" Height="650px" />
    ...
    <div id="divContent" style="display: none; position: absolute; _position: absolute; z-index: 100; border: solid 1px #718CA1; background-color: #F1F1F1; width: 400px; font-size: 13px;">
        <table style="width: 100%;" cellpadding="0" cellspacing="0">
            <tr style="height: 27px; background-image:url(../App_Themes/title.png)">
                <td style="cursor: move; text-align: center; color: white; font-weight: bolder;" valign="middle">批量受理</td>
    ...
    <script type="text/javascript">
        ...
        function Accept(reportId) {
            window.radopen("../CostReport/CostReport_InvoiceAccept.aspx?ReportId=" + reportId, "raw");
        }
