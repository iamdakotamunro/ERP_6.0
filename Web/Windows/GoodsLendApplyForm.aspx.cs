...
using FilialeType = MIS.Enum.FilialeType;
...
namespace ERP.UI.Web.Windows
{
    ...
    /// 添加借出申请
    /// </summary>
    public partial class GoodsLendApplyForm : WindowsPage
    {
        ...
        public string GetShelfType(object shelfType)
        {
        ...
        protected void SelectGoods_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridDataItem dataItem in RGSelectGoods.SelectedItems)
            {
                ...
                {
                    if (!goodsStockList.Any(w => w.RealGoodsId == childGoodsInfo.RealGoodsId && w.ShelfType==(Byte)ShelfType.Normal && w.UnitPrice == purchaseSetInfo.PurchasePrice))
                    {
                        goodsStockList.Add(new StorageRecordDetailInfo
                        {
                            StockId = Guid.Empty,
                            ...
                            ShelfType = (Byte)ShelfType.Normal
                        });
                    }
                }
        ...
        protected void RgGoodsBack_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            ...
            if (OutDetailList.Count > 0)
            {
                ...
                var hostingFilialeId = string.IsNullOrEmpty(RCB_HostingFilialeAuth.SelectedValue)
                    ? Guid.Empty
            ...
            RG_GoodsBack.MasterTableView.Columns.FindByUniqueName("ShelfType").Display = storageType==(byte)StorageType.S;
        }
        ...
        protected void RG_GoodsBack_OnItemCommand(object sender, GridCommandEventArgs e)
        {
            ...
            if (item != null)
            {
                ...
                if (e.CommandName == "QuantityOut")
                {
                    ...
                    var rcbSpecification = (RadComboBox)dataItem.FindControl("rcbSpecification");
                    var shelfType = Convert.ToByte(dataItem.FindControl("ShelfType"));
                    var tbQuantity = (TextBox)dataItem.FindControl("TB_Quantity");
                    var strWarehouseId = RCB_Warehouse.SelectedValue;
                    if (string.IsNullOrEmpty(strWarehouseId) || strWarehouseId == Guid.Empty.ToString())
                    {
        ...
        protected void RgGoods_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            ...
            if (InDetailList.Count > 0)
            {
                ...
                var warehouseIdSelected = new Guid(RCB_Warehouse.SelectedValue);
                Guid hostingFilialeId = !string.IsNullOrWhiteSpace(RCB_HostingFilialeAuth.SelectedValue)
                                ? new Guid(RCB_HostingFilialeAuth.SelectedValue)
                                : Guid.Empty;
                var goodsStockQuantityList = WMSSao.GetGoodsStockByStorageTypeLend(goodsIdOrRealGoodsIdList, warehouseIdSelected, storageType, hostingFilialeId);
                foreach (var info in InDetailList)
                {
                    ...
                    var goodsStock = storageType==(Byte)StorageType.S? goodsStockQuantityList.FirstOrDefault(ent => ent.RealGoodsId == info.RealGoodsId && info.ShelfType==ent.ShelfType)
                        : goodsStockQuantityList.FirstOrDefault(ent => ent.RealGoodsId == info.RealGoodsId);
                    info.NonceWarehouseGoodsStock = goodsStock?.Quantity ?? 0;
                    //info.ShelfType = goodsStock?.ShelfType ?? byte.MinValue;
                }
            }
            ...
            RG_Goods.MasterTableView.Columns.FindByUniqueName("ShelfType").Display = storageType == (byte)StorageType.S;
        }
        ...
        protected void RgGoods_DeleteCommand(object sender, GridCommandEventArgs e)
        {
            ...
            var goodsStockInfo = InDetailList.FirstOrDefault(w => w.RealGoodsId == realGoodsId && w.ShelfType== shelfType && w.UnitPrice == Convert.ToDecimal(unitPrice));
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            ExecuteSubmit((ctx) =>
            {
                ...
                var inStorageRecordInfo = new StorageRecordInfo
                {
                    ...
                    TradeCode = _codeManager.GetCode(CodeType.LO),
                ...
                using (var ts = new TransactionScope(TransactionScopeOption.Required))
                {
                    ...
                    IList<StorageRecordDetailInfo> storageRecordDetailList = (from item in OutDetailList
                                                                              select new StorageRecordDetailInfo
                                                                              {
        ...
        private void GetRgGoodsData()
        {
            ...
            foreach (GridDataItem dataItem in RG_Goods.Items)
            {
                ...
                if (goodsStockList.Count(w => w.RealGoodsId == info.RealGoodsId && w.ShelfType==info.ShelfType && w.UnitPrice == info.UnitPrice) == 0)
        ...
        private void BindStockSearch()
        {
            ...
            if (OutDetailList.Count > 0 && RG_GoodsBack.Visible)
            {
                RG_GoodsBack.Rebind();
            }
        }
        ...
        protected void RgSelectGoodsNeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            ...
            RGSelectGoods.DataSource = GoodsList.OrderBy(ent => ent.GoodsName);
        }
        ...
        protected void DdlShelfTypeSelectedChanged(object sender, EventArgs e)
        {
            ...
            var unitPrice = decimal.Parse(gridDataItem.GetDataKeyValue("UnitPrice").ToString());
        ...
        protected void RgGoodsItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item.ItemType == GridItemType.Item || e.Item.ItemType == GridItemType.AlternatingItem)
            {
                ...
                var storageType = RCB_StorageAuth.SelectedItem != null &&
                    ...
                    : 0;
                if (storageType == (Byte)StorageType.S)
                {
                    ...
                    ddlShelfType.SelectedValue = string.Format("{0}", shelfTypeValue==0?(Byte)ShelfType.Normal: shelfTypeValue);
                }
            }
        }
    }
