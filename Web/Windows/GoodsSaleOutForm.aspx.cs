...
namespace ERP.UI.Web.Windows
{
    ...
    /// 添加销售出库
    /// </summary>
    public partial class GoodsSaleOutForm : WindowsPage
    {
        ...
        protected void RcbStorageAuthOnSelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                new HostingFilialeAuth { HostingFilialeId = Guid.Empty, HostingFilialeName = "" }
            };
            var warehouseAuth = CurrentSession.Personnel.WarehouseList.FirstOrDefault(p => p.WarehouseId == warehouseId);
        ...
        protected void SelectGoods_Click(object sender, EventArgs e)
        {
            ...
            var goodsTypes = WMSSao.GetPurchaseGoodsTypes(new Guid(strWarehouseId), new Guid(strHostingFilialeId));
            foreach (GridDataItem dataItem in RGSelectGoods.SelectedItems)
            {
        ...
        protected void btn_InsterStock(object sender, EventArgs e)
        {
            ...
            ExecuteSubmit((ctx) =>
            {
                ...
                string description = string.Format("[销售出库;出库人:{0};出库单备注:{1};{2}]", personnelInfo.RealName, storageRecordDescription, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));


                var tradeCode = _code.GetCode(CodeType.SL);
