...
namespace ERP.UI.Web.Windows
{
    ...
    /// 重送借出返回
    /// </summary>
    public partial class EditGoodsLendApplyReturnForm : System.Web.UI.Page
    {
            ...
            if (OutDetailList.Count > 0)
            {
                ...
                if (wList != null)
                {
                    ...
                    if (slist != null)
                    {
                        ...
                        if (slist.IsReal)
                        {
                            hostingFilialeId = new Guid(RCB_HostingFilialeAuth.SelectedValue);
                        }
                ...
                foreach (var info in OutDetailList)
                {
                    ...
                    if (goodsStockQuantityList != null && goodsStockQuantityList.Count > 0)
                    {
                        //可出库数
                        var goodsStockKeyValuePair =
                            goodsStockQuantityList.FirstOrDefault(w => w.Key == info.RealGoodsId);
