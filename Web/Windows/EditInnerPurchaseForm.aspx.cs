...
{
    public partial class EditInnerPurchaseForm : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                isAgain = Request.QueryString["IsAgain"];
        ...
        private void BindData(Guid stockId)
        {
            ...
            InStockId = InnerPurchase.InStockId;
            PurchasingId = InnerPurchase.PurchasingId;
            ...
            foreach (var info in InDetailList)
            {
                ...
                info.GoodsType = goodsInfo.GoodsType;
            ...
            RCB_OutWarehouse.SelectedValue = storageRecordinfo.WarehouseId.ToString();
            RcbOutStockChanged();
            RCB_OutStorageAuth.SelectedValue = storageRecordinfo.StorageType.ToString();
            RcbOutStorageAuthChanged(false);
            RCB_OutHostingFilialeAuth.SelectedValue = storageRecordinfo.FilialeId.ToString();
            RCB_InWarehouse.SelectedValue = InnerPurchase.InWarehouseId.ToString();
            RcbInStockChanged();
            RCB_InStorageAuth.SelectedValue = InnerPurchase.InStorageType.ToString();
            RcbInStorageAuthChanged(false);
            RCB_InHostingFilialeAuth.SelectedValue = InnerPurchase.InHostingFilialeId.ToString();
        ...
        private void RcbOutStockChanged()
        {
        ...
        {
            DeleteSelectGoodsList();
            RcbOutStorageAuthChanged(true);
        }

        private void RcbOutStorageAuthChanged(bool sel)
        {
            //仓库id
        ...
        {
            RcbInStorageAuthChanged(true);
        }

        private void RcbInStorageAuthChanged(bool sel)
        {
            //仓库id
        ...
        protected void SelectGoods_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridDataItem dataItem in RGSelectGoods.SelectedItems)
            {
                ...
                foreach (var childGoodsInfo in childGoodsList)
                {
                    ...
                    var goodsStockInfo = new StorageRecordDetailInfo
                    {
                        ...
                        UnitPrice = realTimeGrossSettlementManager.GetLatestUnitPrice(new Guid(RCB_OutHostingFilialeAuth.SelectedItem.Value), goodsInfo.GoodsId) * (internalPriceSet == null ? 0 : Convert.ToDecimal(internalPriceSet.ReserveProfitRatio)),
                        Quantity = 0,
            ...
            foreach (var info in goodsStockList)
            {
                if (dicGoods != null && dicGoods.Count > 0)
                {
                    ...
                    if (hasKey)
                    {
                        var goodsBaseInfo = dicGoods.FirstOrDefault(w => w.Key == info.RealGoodsId).Value;
                        if (goodsBaseInfo.ExpandInfo != null)
                        {
                            //获取加盟价
                            info.UnitPrice = goodsBaseInfo.ExpandInfo.JoinPrice;
                        }
                    }
                }
            }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void btn_UpdateStock(object sender, EventArgs e)
        {
            ...
            //出入库详细
            var goodsStockList = GetRgGoodsData(OutStockId);//stockId
            ...
            var inCompanyId =_companyCussent.GetCompanyIdByRelevanceFilialeId(new Guid(RCB_InHostingFilialeAuth.SelectedValue));
            if (inCompanyId==Guid.Empty)
            {
                RAM.Alert("入库物流公司未关联往来单位！");
                return;
            }
            using (var ts = new TransactionScope(TransactionScopeOption.Required))
            {
                try
                {
                    ...
                    if (isAgain == "1")
                    {
                        ...
                        _storageManager.UpdateStorageRecordAndStorageRecordDetail(outStockInfo, goodsStockList);
                        //购买入库添加操作记录添加
        ...
        private String isAgain
        {
            get
            {
                if (ViewState["isAgain"] == null)
                    return String.Empty;
                return (String)ViewState["isAgain"];
            }
            set { ViewState["isAgain"] = value; }
        }
        ...
        private String No
        {
            get
            {
                if (ViewState["No"] == null)
                    return String.Empty;
                return (String)ViewState["No"];
            }
            set { ViewState["No"] = value; }
        }
        ...
        private Guid InStockId
        {
            get
            {
                if (ViewState["InStockId"] == null)
                    return Guid.Empty;
                return (Guid)ViewState["InStockId"];
            }
        ...
        private Guid PurchasingId
        {
            get
            {
                if (ViewState["PurchasingId"] == null)
                    return Guid.Empty;
                return (Guid)ViewState["PurchasingId"];
            }
            set { ViewState["PurchasingId"] = value; }
        }
